                                        ...
                                        HYPRE_Int G_type )
{
   ...
   else
   {
      ...
      counts = (HYPRE_Int *) sicm_alloc(nrows_G * sizeof(HYPRE_Int));
      for (i = 0; i < nrows_G; i++) counts[i] = 0;
      for (i = 0; i < T_diag_i[ncols_G]; i++) counts[T_diag_j[i]]++;
      G_diag_i = (HYPRE_Int *) sicm_alloc((nrows_G+1) * sizeof(HYPRE_Int));
      G_diag_j = (HYPRE_Int *) sicm_alloc(T_diag_i[ncols_G] * sizeof(HYPRE_Int));
      G_diag_i[0] = 0;
   ...
   GT_diag_mat = (HYPRE_Int *) sicm_alloc(2 * ncols_G * sizeof(HYPRE_Int));
   ...
   nodes_marked = (HYPRE_Int *) sicm_alloc(nrows_G * sizeof(HYPRE_Int));
   edges_marked = (HYPRE_Int *) sicm_alloc(ncols_G * sizeof(HYPRE_Int));
   for (i = 0; i < nrows_G; i++) nodes_marked[i] = 0; 
   for (i = 0; i < ncols_G; i++) edges_marked[i] = 0; 
   queue = (HYPRE_Int *) sicm_alloc(nrows_G * sizeof(HYPRE_Int));
   queue_head = 0;
   queue_tail = 1;
   queue[0] = 0;
   nodes_marked[0] = 1;
   ...
   sicm_free(nodes_marked);
   sicm_free(queue);
   sicm_free(GT_diag_mat);
   ...
   if (nprocs > 1)
   {
      ...
      {
         n_proc_array = 0;
         proc_array = (HYPRE_Int *) sicm_alloc((nsends+nrecvs) * sizeof(HYPRE_Int));
      ...
      pgraph_i = (HYPRE_Int *) sicm_alloc((nprocs+1) * sizeof(HYPRE_Int));
      recv_cnts = (HYPRE_Int *) sicm_alloc(nprocs * sizeof(HYPRE_Int));
      hypre_MPI_Allgather(&n_proc_array, 1, HYPRE_MPI_INT, recv_cnts, 1,
                          HYPRE_MPI_INT, comm);
      pgraph_i[0] = 0;
      for (i = 1; i <= nprocs; i++)
         pgraph_i[i] = pgraph_i[i-1] + recv_cnts[i-1];
      pgraph_j = (HYPRE_Int *) sicm_alloc(pgraph_i[nprocs] * sizeof(HYPRE_Int));
      ...
      nodes_marked = (HYPRE_Int *) sicm_alloc(nprocs * sizeof(HYPRE_Int));
      for (i = 0; i < nprocs; i++) nodes_marked[i] = -1; 
      queue = (HYPRE_Int *) sicm_alloc(nprocs * sizeof(HYPRE_Int));
      queue_head = 0;
      queue_tail = 1;
      node = 0;
      queue[0] = node;
      ...
      {
         children = (HYPRE_Int *) sicm_alloc(n_children * sizeof(HYPRE_Int));
         n_children = 0;
         for (i = 0; i < nprocs; i++) 
   ...
   if (n_children > 0) sicm_free(children);
   ...
   t_indices = (HYPRE_Int *) sicm_alloc((tree_size+1) * sizeof(HYPRE_Int));
   t_indices[0] = tree_size;
   tree_size = 1;
   for (i = 0; i < ncols_G; i++)
      if (edges_marked[i] == 1) t_indices[tree_size++] = i;
   (*indices) = t_indices;
   sicm_free(edges_marked);
   if (G_type != 0)
   {
      sicm_free(G_diag_i);
      sicm_free(G_diag_j);
                                           ...
                                           hypre_ParCSRMatrix ***submatrices )
{
   ...
   proc_offsets1 = (HYPRE_Int *) sicm_alloc((nprocs+1) * sizeof(HYPRE_Int));
   proc_offsets2 = (HYPRE_Int *) sicm_alloc((nprocs+1) * sizeof(HYPRE_Int));
   ...
   exp_indices = (HYPRE_Int *) sicm_alloc(nrows_A * sizeof(HYPRE_Int));
   ...
   sicm_free(proc_offsets1);
   sicm_free(proc_offsets2);
   sicm_free(exp_indices);
}
