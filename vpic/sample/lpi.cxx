...
//  int use_dump	    = 0;
  ...
  // BJA - autoscale problem size to nproc()
  float box_size_x =  nproc() * ( 120.0*1e-4 ) / 1296.0;
  float box_size_z =  12.0*1e-4;

  int mobile_ions           = 1;      // Whether or not to push ions
  double nppc               = 8192; // lucho 512;     // Ave. number of particles/cell in ea. species
...
//e0                       = e0*(waist/width);    // at entrance (3D Gaussian)
  ...
  double t_stop            = 0.05*wpe1ps;               // Runtime in 1/wpe
  int particle_interval    = 0; // int(0.01*wpe1ps/dt); // 0;
  int poynting_interval    = 0; // int(M_PI/((omega+1.5)*dt));                  // Num. steps between dumping poynting flux
  int field_interval       = 0; // int(0.01*wpe1ps/dt);         // Num. steps between saving field, hydro data
  int velocity_interval    = 0; // int(0.1*wpe1ps/dt);        // How frequently to dump binned velocity space data
  int restart_interval     = 10000000;       // Num. steps between restart dumps
  int quota_check_interval = 0; // 20;
...
//  global->use_unity	       = use_unity;
//  global->use_dump	       = use_dump;
  global->e0                   = e0;
  global->omega                = omega;
...
//  if (use_unity) {
//	unity_setup_dataset(nx, ny, nz, 1, 488);
//	unity_setup_particles();
//  }
...
# define should_dump(x) \
  ...
  if ( should_dump(particle) && global->load_particles ) {
    ...
    if ( global->mobile_ions ) {
      if ( global->H_present  ) {
		dump_particles( "H",  "particle/Hparticle" );
	}
      }
      if ( global->He_present ) {
		dump_particles( "He", "particle/Heparticle" );
	}
      }
    }
  }
